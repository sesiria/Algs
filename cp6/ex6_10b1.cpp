...
* Exercise 6.10b1
* Author: sesiria  2018
* print all element less than k from an leftist heap
*/
#include <algorithm>
#include <iostream>
...
class LeftistHeap
{
  ...
  private:
    void printElements(LeftistNode *node, const Comparable &x);
 ...
 */
template <typename Comparable>
void LeftistHeap<Comparable>::printElements(const Comparable &x)
{
    if (isEmpty())
        return;
    printElements(root, x);
}

template <typename Comparable>
void LeftistHeap<Comparable>::printElements(LeftistNode *t, const Comparable &x)
{
    if(t == nullptr)
        return;
    if(t->element < x) {
        std::cout << t->element << " ";
        printElements(t->left, x);
        printElements(t->right, x);
    }
}
