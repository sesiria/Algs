...
std::vector<std::vector<char>> vec ={
	std::vector<char>{'t', 'h', 'i', 's'},
	std::vector<char>{'g', 't', 't', 's'},
	std::vector<char>{'o', 'a', 'h', 'w'},
	std::vector<char>{'w', 'f', 't', 's'}
};
    ...
    const matrix<char> & mar, int x_begin, int y_begin, int x_end, int y_end)
{
     char buf[BUFSIZ] = {0};
    std::string str;
    int offsetX = (x_begin < x_end) ? 1 : 
        (x_begin > x_end) ? -1 : 0;
    int offsetY = (y_begin < y_end) ? 1 :
        (y_begin > y_end) ? -1 : 0;
    int xGuard = x_end + offsetX;
    int yGuard = y_end + offsetY;
    int pos = 0;
    for(int i = x_begin, j = y_begin
        ; i != xGuard || j != yGuard;
        i+= offsetX, j+= offsetY)
    {
        buf[pos++] = mar[i][j];
...
{
    int offsetX = (x_start < x_end) ? 1 : 
        (x_start > x_end) ? -1 : 0;
    int offsetY = (y_start < y_end) ? 1 : 
        (y_start > y_end) ? -1 : 0;
