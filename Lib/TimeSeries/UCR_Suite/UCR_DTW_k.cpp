...
// comparator
struct cmp {
	bool operator()(const pair<double, long long > & l, const pair<double, long long > &r) {
...
int main(int argc, char *argv[])
{
	...
	priority_queue<pair<double, long long>, vector<pair<double, long long >>, cmp> maxHeap;
	maxHeap.push(make_pair(INF, -1));
		...
		else
		{
			...
			for (i = 0; i < ep; i++)
			{
				...
				if (i >= m - 1)
				{
					...
					bsf = maxHeap.top().first;

					/// Use a constant lower bound to prune the obvious subsequence
					lb_kim = lb_kim_hierarchy(t, q, j, m, mean, std, bsf);

					if (maxHeap.size() < candidates) {
						dist = dtw(tz, q, cb, m, r, bsf);
						loc = loc = (it)*(EPOCH - m + 1) + i - m + 1;
						maxHeap.push(make_pair(dist, loc));
					}
					else if (lb_kim < bsf)
					{
	...
	int count = 1;
	while (!maxHeap.empty()) {
		cout << "candidates" << count++ << " "<< "distance:\t" << sqrt(maxHeap.top().first)
			<< "\t loc:\t" << maxHeap.top().second << endl;
		maxHeap.pop();
	}
	return 0;
}
